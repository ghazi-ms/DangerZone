...
class _CardsState extends State<Cards> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var dangerElement in dangerZonesData) {
      for (var historyElement in historyList) {
        if (dangerElement['id'].toString() == historyElement['id'].toString()) {
          bool match = false;
          for (var matchedElement in matchedList) {
            if (matchedElement['id'] == historyElement['id']) {
              match = true;
            }
          }
          if (!match) {
            matchedList.insert(0,{
              'Coordinates': historyElement['position'],
              'Locations': dangerElement['Locations'],
              'description': dangerElement['description'],
              'id': dangerElement['id'].toString(),
              'timeStamp': dangerElement['timeStamp'],
              'title': dangerElement['title'],
            });
            dangerElement['Coordinates'] = historyElement['position'];
          }
        }
      }

    }
    void deleteCard(var id){
      for(int index=0;index<matchedList.length;index++){
        if (matchedList[index]['id'].toString()==id.toString()) {
          matchedList.remove(matchedList[index]['id']);
        }
      }
    }
                  ...
                  for (int index = 0; index < matchedList.length; index++)
                    Padding(
                      ...
                      child: NewsCards(
                        ...
                        coordinate: matchedList[index]['Coordinates'],
                        id: matchedList[index]['id'].toString(),
                      ),
                    ),
                  NewsCards(
                    title:
